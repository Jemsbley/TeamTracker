...
function ReportEditor({ report }: { report: ScoutingReport }) {
  ...
  // Backfill maps for older reports so every map has a row.
  useEffect(() => {
    const haveAll =
  ...
  const setMaps = (maps: ScoutMap[]) => update(report.id, { maps });
  ...
  return (
    <div className="space-y-4">
      <Link
        to="/scouting"
        className="text-sm text-valorant-muted hover:text-valorant-accent"
      >
        ← Scouting
      </Link>
